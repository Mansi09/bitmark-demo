...
const main = async () => {
     ...
     */
    const assetName = 'MANSI_TEST_PROPERTY'; // Asset length must be less than or equal 64 characters
    const assetFilePath = '/Users/openxcell/bitmark-demo-v1/bitmark-sdk-js/sample/main.js';
    ...
    const quantity = 1; // Number of bitmarks you want to issue, quantity must be greater than or equal 1.
    ...
    console.log("bitmarkQueryParams", bitmarkQueryParams)
    let yourBitmarks = await queryBitmarks(bitmarkQueryParams);

    console.log("yourBitmarks", yourBitmarks)
    ...
    console.log("yourTransactions", yourTransactions)

    // 3.2.2 Query Transaction
    const txId = yourTransactions[0].id;
    ...
    console.log("yourAssets", yourAssets)

    // 3.3.2 Query asset
    assetId = yourAssets[0].id;
